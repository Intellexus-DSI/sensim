...
max_seq_length=("512")
warmup_steps=("64" "256")
weight_decay=("0.5" "0.1" "0.001")
batch_size=("32") # Fixed!
gradient_accumulation_steps=("8") # Fixed!
...
run_suffix="_grid_search2"
shared_result_filename="llms_sets_results_${run_time_identifier}_${synthetic_row_count}${run_suffix}.csv"
...
run_experimentOnSBERT() {
    ...
    echo "Arguments (merge=$merge_flag, set=$set_letter, mode=$train_mode, unicode=$use_unicode_columns, no_fit=$no_fit): $model $pooling $epochs $lr $loss_type $batch_size $warmup_steps $max_seq_length $loss_scale $weight_decay $gradient_accumulation_steps $lr_scheduler_type"
    ...
    [ "$run_aggregate" = true ] && python -m sub_tasks.aggregate_sets --run_time_identifier "${run_time_identifier}_${synthetic_row_count}${run_suffix}"
}
