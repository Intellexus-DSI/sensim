...
# Required for SentencePiece-based tokenizers (e.g. hfl/cino-large-v2) with newer protobuf versions
...
modals=("buddhist-nlp/byt5-mitra-bo")
#modals=("sangjeedondrub/tibetan-roberta-base")
#modals=("sangjeedondrub/tibetan-roberta-base" "CMLI-NLP/TiBERT" "hfl/cino-large-v2" "sentence-transformers/LaBSE")
...
use_unicode_columns=true #true  # If true, passes --use_unicode_columns to use SentenceA_unicode/SentenceB_unicode columns
...
#run_time_identifier="2026-02-27_12-24-40" # 5,000 mined, minimum_faiss 60, local_models
run_time_identifier="2026-02-27_22-20-37" # 5,000 mined, minimum_faiss 60, BWS (Claude + Gemini) * 2
#run_time_identifier="2026-02-27_22-20-37-2026-02-27_12-24-40" # Total 10,000 mined, minimum_faiss 60, BWS (Claude + Gemini) * 2 + local_models
...
synthetic_row_count=$(python -c "import pandas as pd; print(len(pd.read_excel('${shared_data_dir}/${shared_train_filename_synthetic}')))")
shared_result_filename="llms_sets_results_${run_time_identifier}_${synthetic_row_count}.csv"
shared_model_dir_synthetic="ckpts/sts-b/synthetic"
shared_model_dir=("ckpts/sts-b/${run_time_identifier}_unicode/${run_timestamp}")

[ "$no_fit" = true ] && shared_result_filename="llms_no_fit_unicode.csv"
...
run_experimentOnSBERT() {
    ...
    [ "$run_aggregate" = true ] && python -m sub_tasks.aggregate_sets --run_time_identifier "${run_time_identifier}_${synthetic_row_count}"
}
