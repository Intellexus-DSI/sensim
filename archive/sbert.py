...
def main(args_list=None):
    ...
    train_loss = None  # Initialize train_loss outside the if block

    # --- FIRST FIT (Train 1 / Synthetic Set) ---
    if not args.no_fit:
        print("Starting fine-tuning (Train 1) with SentenceTransformer...")

        train_samples = [
            InputExample(texts=[row['text1'], row['text2']], label=float(row['label']))
            for row in train_ds
        ]
        train_dataloader = DataLoader(train_samples, shuffle=True, batch_size=fit_config['batch_size'])
        ...
        train_metrics = model_utils.bi_encoder_evaluate(model, train_ds, batch_size=4)
        validation_metrics = model_utils.bi_encoder_evaluate(model, valid_ds, batch_size=4)
        test_metrics = model_utils.bi_encoder_evaluate(model, test_ds, batch_size=4)

        test2_metrics = None
        if test2_ds is not None:
            test2_metrics = model_utils.bi_encoder_evaluate(model, test2_ds, batch_size=4)
        ...
        print(f"First fit results logged to: {log_results_file}")
    ...
    if not args.no_fit and train2_ds is not None:
        print("Starting second fine-tuning (Train 2) with SentenceTransformer...")

        train2_samples = [
            InputExample(texts=[row['text1'], row['text2']], label=float(row['label']))
            for row in train2_ds
        ]

        train2_dataloader = DataLoader(train2_samples, shuffle=True, batch_size=fit_config['batch_size'])

        # train_loss is defined in the previous block
        model.fit(
            train_objectives=[(train2_dataloader, train_loss)],
            epochs=fit_config['epochs'],
            optimizer_params={'lr': fit_config['learning_rate']},
            warmup_steps=fit_config['warmup_steps'],
            output_path=args.model_dir,
        ...
        train_metrics_2 = model_utils.bi_encoder_evaluate(model, train_ds, batch_size=4)
        print('Train set pearson (FINAL):', train_metrics_2['pearson_cosine'])

        validation_metrics_2 = model_utils.bi_encoder_evaluate(model, valid_ds, batch_size=4)
        print('Validation set pearson (FINAL):', validation_metrics_2['pearson_cosine'])

        test_metrics_2 = model_utils.bi_encoder_evaluate(model, test_ds, batch_size=4)
        print('Test set pearson (FINAL):', test_metrics_2['pearson_cosine'])

        test2_metrics_2 = None
        if test2_ds is not None:
            print("Evaluating SECOND test set (FINAL)...")
            test2_metrics_2 = model_utils.bi_encoder_evaluate(model, test2_ds, batch_size=4)
        ...
        # Use prefix "2" to generate column names like "train2_spearman"
