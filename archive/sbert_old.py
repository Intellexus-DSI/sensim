...
def main(args_list=None):
    ...
    # fitting
    if not args.no_fit:
        print("Starting fine-tuning with SentenceTransformer...")
        ...
        # Create DataLoader using fit_config batch size
        ...
        # Run fine-tuning using parser-based config
        model.fit(
    ...
    #train_metrics = evaluate(model, train_ds, batch_size=4)
    train_metrics = model_utils_old.bi_encoder_evaluate(model, train_ds, batch_size=4)
    print('Train metrics:', train_metrics)
    print('Train set pearson:', train_metrics['pearson_cosine'])

    #validation_metrics = evaluate(model, valid_ds, batch_size=4)
    validation_metrics = model_utils_old.bi_encoder_evaluate(model, valid_ds, batch_size=4)
    print('Validation metrics:', validation_metrics)
    print('Validation set pearson:', validation_metrics['pearson_cosine'])

    #test_metrics = evaluate(model, test_ds, batch_size=4)
    test_metrics = model_utils_old.bi_encoder_evaluate(model, test_ds, batch_size=4)
    print('Test metrics:', test_metrics)
    print('Test set pearson:', test_metrics['pearson_cosine'])
    # ---- Test 2 (optional) ----
    test2_metrics = None
    if test2_ds is not None:
        test2_metrics = model_utils_old.bi_encoder_evaluate(model, test2_ds, batch_size=4)
