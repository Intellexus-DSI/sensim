...
def main(args_list=None):
    ...
    # fitting
    if not args.no_fit:
        print("Starting fine-tuning with CrossEncoder...")

        # --- CRITICAL FIX: Filter out invalid data rows ---

        # 1. Filter out rows where text1 or text2 are empty, missing, or not strings.
        # This prevents the tokenizer from failing and returning dimension-0 tensors.
        filtered_train_ds = [
        ...
        if len(filtered_train_ds) < len(train_ds):
            print(f"Warning: Removed {len(train_ds) - len(filtered_train_ds)} rows with empty/invalid text inputs.")

        # Convert your dataset rows into SBERT InputExamples
        train_samples = [
            InputExample(texts=[row['text1'], row['text2']], label=float(row['label']))
            for row in filtered_train_ds # <-- Use the filtered list here
        ]
        # --- END CRITICAL FIX ---

        # 1. REQUIRED: Create DataLoader (this is now the main argument)
        ...
        # 2. REQUIRED: Define the loss function (needed for the 'loss_fct' argument)
        train_loss = losses.MSELoss(model)

        # 3. MODIFIED: Call fit using positional and specific keyword arguments
        model.fit(
            # Pass DataLoader as the FIRST POSITIONAL ARGUMENT
            train_dataloader,

            # Pass loss function via the expected keyword argument
            loss_fct=nn.MSELoss(),
            ...
            # 4. CRITICAL FIX: Pass LR via the expected dictionary structure
    ...
    train_metrics = model_utils_old.cross_encoder_evaluate(model, train_ds, batch_size=4)
    print('Train metrics:', train_metrics)
    print('Train set pearson:', train_metrics['pearson_cosine'])

    validation_metrics = model_utils_old.cross_encoder_evaluate(model, valid_ds, batch_size=4)
    print('Validation metrics:', validation_metrics)
    print('Validation set pearson:', validation_metrics['pearson_cosine'])

    test_metrics = model_utils_old.cross_encoder_evaluate(model, test_ds, batch_size=4)
    ...
    if test2_ds is not None:
        test2_metrics = model_utils_old.cross_encoder_evaluate(model, test2_ds, batch_size=4)
        print('Test2 metrics:', test2_metrics)
        print('Test2 pearson:', test2_metrics['pearson_cosine'])
    ...
    if args.pool_of_pairs_filename:
        model_utils_old.calculate_pairs_cosine(model, os.path.join(args.results_dir, args.pool_of_pairs_filename), os.path.join(args.results_dir, args.pool_of_pairs_cosine_filename or args.pool_of_pairs_filename))
